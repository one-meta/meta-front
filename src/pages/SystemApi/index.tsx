...
import {
  ...
  getSystemapi,
  postSystemapi,
  postSystemapiBulk__openAPI__delete,
  putSystemapiId,
} from '@/services/meta/SystemApi';
import { transformTime } from '@/utils/transform';
import { actRadioEnum } from '@/utils/util';
import {
  ActionType,
  ProColumns,
  ProForm,
...
const IndexPage: React.FC = () => {
  ...
  //多行选择
  const [selectedRowsState, setSelectedRows] = useState<API.SystemApi[]>([]);
  ...
  const newPageDetail = (
    <ProForm.Group>
      ...
      <ProFormRadio.Group
        ...
        radioType="button"
      />
      <ProFormText name="path" label="路径" rules={[{ required: true }]} width="xl" />
      <ProFormTextArea
        name="roles"
        label="角色"
        rules={[{ required: true }]}
        placeholder="一行一条数据"
      />
      <ProFormSwitch name="public" label="公共路由" />
      <ProFormSwitch name="sa" label="仅sa可操作" />
  ...
  //增加行操作：编辑、查看、删除
  const columns: ProColumns<API.SystemApi>[] = [
    ...systemApiColumns,
    {
      title: '操作',
      valueType: 'option',
      ...
      render: (_text, record) => [
        ...
        //详情（跳转页面）
        // TODO：根据需求修改路径
        // <LinkLayout key="link" to={`/permission/api/detail/${record.id}`} />,
        //查看
        <ShowRowDetailLayout
          key="view"
  ...
  return (
    <ProTableLayout
      ...
      columns={columns}
      //Service 方法
      getMethod={getSystemapi}
      editMethod={putSystemapiId}
      newMethod={postSystemapi}
      deleteBulkMethod={postSystemapiBulk__openAPI__delete}
